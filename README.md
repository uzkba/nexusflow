...
A arquitetura desacopla ingestão de dados, curadoria de dados (com aprovação humana) e interface de visualização, priorizando confiabilidade do dado apresentado sobre velocidade de entrega.

Projeto de uso interno: não há cliente externo acessando o sistema. Foi encomendado por um funcionário para facilitar o próprio trabalho e o dos colegas de equipe — todos os usuários do painel são internos, com o mesmo papel (`admin`).

A consolidação de clientes duplicados na base (mesma empresa aparecendo com nomes ligeiramente diferentes) não é automática: o sistema sugere agrupamentos e a equipe aprova ou rejeita cada sugestão antes que ela vire dado oficial no dashboard.
...
4. **Aprovação (interna)** — tela dedicada do painel onde a equipe vê cada sugestão, o score e os projetos envolvidos, e aprova ou rejeita. Decisão registrada com autor e data/hora.
...
| cegs_relacionados | array / jsonb | Contexto para quem decide |
...
### `usuarios`
Autenticação do painel — uso interno da equipe. Não há conta de cliente externo; único papel usado hoje é `admin`.

| Coluna | Tipo | Notas |
|---|---|---|
| id | UUID (PK) | |
| email | text (unique) | Validação de formato feita na camada de API (Pydantic `EmailStr`), não no banco — mesma lógica de `senha_hash`: o schema guarda `text`, a biblioteca aplicável valida/transforma antes de persistir |
| senha_hash | text | bcrypt |
| papel | enum (admin) | NOT NULL, default 'admin' — único valor usado hoje; mantido como enum (não boolean) para comportar evolução futura sem exigir migration estrutural |
| ativo | boolean | default true — permite revogar acesso sem apagar o registro (preserva FKs de decisões já tomadas) |
| criado_em | timestamp | |
| atualizado_em | timestamp | |

### `refresh_tokens`
Suporte à rotação e revogação de refresh tokens (login persistente sem token stateless de longa duração).

| Coluna | Tipo | Notas |
|---|---|---|
| id | UUID (PK) | |
| usuario_id | FK → usuarios.id | ON DELETE CASCADE |
| token_hash | text (unique) | SHA-256 do token — valor puro nunca é persistido |
| criado_em | timestamp | |
| expira_em | timestamp | |
| revogado_em | timestamp | Nulo até logout, rotação ou revogação em massa |
| substituido_por_id | UUID | Aponta para o novo registro gerado na rotação, se houver |
...
|---|---|---|
| `/api/auth/login` | POST | Autenticação. Não retorna tokens no corpo — define access token e refresh token como cookies HttpOnly/Secure/SameSite=Strict |
| `/api/auth/refresh` | POST | Rotaciona o refresh token (cookie) e emite novo access token + novo refresh token, ambos como cookies |
| `/api/auth/logout` | POST | Revoga o refresh token atual e limpa os cookies de sessão |
...
Todas as rotas — exceto `/api/auth/login` e `/api/auth/refresh` — exigem access token válido (cookie HttpOnly). `/api/auth/refresh` exige apenas o cookie de refresh token, não o access token (é justamente a rota chamada quando o access token expirou). Todos os endpoints analíticos recebem parâmetros de filtro (origem, cliente, uf, município, ano) e devolvem agregação já processada no banco, nunca dado bruto para o frontend somar.
...
- **Aprovação de Consolidação** — lista de sugestões de fusão de clientes, com score de similaridade e CEGs relacionados, para a equipe aprovar ou rejeitar. Tela interna, com a mesma qualidade visual (Shadcn/UI) das demais abas.
...
- ~~Modelar tabela de usuários e estratégia de autenticação (JWT + refresh token)~~ — concluído, ver `Modelagem_Usuarios_Autenticacao_JWT.docx`
